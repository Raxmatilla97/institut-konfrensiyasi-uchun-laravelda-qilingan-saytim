...
<div class="card">
    <div class="card-header">
       {{ $konf->first_name }} {{ $konf->last_name }} {{ $konf->middle_name }}ning anketasini o'zgartirish. | {{ $konf->first_name }} {{ $konf->last_name }} {{ $konf->middle_name }} изменить анкету. | {{ $konf->first_name }} {{ $konf->last_name }} {{ $konf->middle_name }} change the questionnaire
    </div>

    <div class="card-body">
        <form action="{{ route('admin.home.update', [$konf->id]) }}" method="POST" enctype="multipart/form-data">
            @csrf
            @method('PUT')
            ...
            <div class="form-group {{ $errors->has('gender') ? 'has-error' : '' }}">
                ...
                <select class="form-control" id="type" name="gender">
                    @if($konf->gender == 0)
                    <option value="0" selected>Ayol | Женщина | Woman</option>
                    @else($konf->gender == 1)
                    <option value="1"  selected>Erkak | Мужчина | Male </option>
                    @endif

                    @if($konf->gender == 0)
                    <option value="1">O'zgartirilsin -> Erkakga | Изменить -> Мужчине | Change -> To a man</option>
                    @else($konf->gender == 1)
                    <option value="0">O'zgartirilsin -> Ayolga | Изменить -> Женщина | Change -> To a woman</option>
                    @endif
            ...
            <div class="form-group {{ $errors->has('position') ? 'has-error' : '' }}">
                ...
                <option value="{{ old('position', isset($konf) ? $konf->position : '') }}" selected>{{ old('position', isset($konf) ? $konf->position : '') }}</option>
                    <option value="none" >Belgilang</option>
            ...
            <div><input class="btn btn-success" type="submit" value="Saqlang va yoping | Сохрани и закрой | Save and close"></div>
    </form>
